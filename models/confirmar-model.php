...
class ConfirmarModel
{
  ...
  public function create($data, $gestion=2021) {
    ...
    if($this->verifyCreateEnabled($id_not_pro, $gestion)) {
      if(!$this->verifyCreateRepeat($id_not_pro)) {
  ...
  public function getEstudiante($ci, $gestion=2021) {
    ...
    WHERE NP.gestion = $gestion AND EST.ci = '$ci'";
    $query = $conex->prepare($sql);
    $query->execute();
    if ($query->rowCount() != 0) {
  ...
  private function verifyCreateRepeat($id_not_pro) {
    $conex = $this->pdo;
    $sql = "SELECT *
  ...
  private function verifyCreateEnabled($id_not_pro, $gestion=2021) {
    $conex = $this->pdo;
    // $gestion = $this->entorno->getGestion();
    $sql = "SELECT *
      FROM nota_prom
